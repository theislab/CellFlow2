...
class DAGClassLoader:
    ...
    def _derive_child_schedule(self, child: str, root_sched: np.ndarray) -> np.ndarray:
        ...
        When several child leaves share the bound value — i.e. the child partitions on columns beyond
        ``common`` (e.g. child cols ``(a, x)`` bound on ``a``) — one is drawn ∝ the child's leaf
        weights, so ``P(child extra cols | common)`` is controlled by the child weights, like the root.
        Conditioning is required: an unmatched shared-column value raises (no silent fallback); declare
        ``Bind(..., common=())`` for explicit unconditional sampling.
        """
